...
const Navbar = ({ currentPage, currentCategoryId, navigate, cartCount, openCart, openSearch, categories }) => {
  ...
  const bottomNav = ReactDOM.createPortal(
    <nav className="store-bottom-nav">
      ...
      <button className="store-bottom-nav-item" onClick={openSearch}>
      ...
      <button className={'store-bottom-nav-item' + (currentPage === 'about' ? ' active' : '')} onClick={() => navigate({ page: 'about' })}>
        <i className="ph ph-info" /><span>About</span>
      </button>
    </nav>,
    document.body
  );
